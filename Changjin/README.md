# Chemical Similarity Search API

This project is a high-performance cheminformatics API built with FastAPI, RDKit, and DuckDB. It allows users to search for chemically similar molecules using a SMILES string and supports filtering based on molecular properties.

## Core Technology Stack

* **Web Framework**: [FastAPI](https://fastapi.tiangolo.com/) - For building high-performance, easy-to-use APIs.
* **Database**: [DuckDB](https://duckdb.org/) - A fast, in-process analytical database, perfect for handling and querying the data in this project.
* **Cheminformatics**: [RDKit](https://www.rdkit.org/) - The industry-standard open-source cheminformatics toolkit for processing molecules, calculating fingerprints, and similarity.
* **Data Validation**: [Pydantic](https://docs.pydantic.dev/) - Used to define clear, robust data models and handle data validation and documentation automatically.

## Project Structure

The project's code structure follows the principle of separation of concerns, organizing different functionalities into independent modules.

Affordable/
├── app/
│   ├── main.py        # FastAPI application entry point, contains all API routes
│   ├── models.py      # Pydantic data models, defining API request/response structures
│   ├── core/
│   │   ├── chem.py    # Wraps core RDKit chemical calculation logic
│   │   └── db.py      # Helper functions for database connection and queries
│   └── ...            # Other frontend files (index.html, css, js)
│
├── scripts/
│   └── import_chembl.py # [IMPORTANT] One-time script for data preprocessing and database creation
│
├── chembl_data.duckdb   # The database file generated by the script
├── sample_chembl.csv    # The original sample data source
└── requirements.txt     # The project's Python dependency list

## Development Logic & Setup Steps

The project is designed to be run in two main phases: a **one-time data preprocessing step** and **launching the API service**.

### Phase 1: Data Preprocessing & Database Creation (One-Time Operation)

This step is required when running the project for the first time or when the data source is updated. This process reads the raw data file (e.g., `sample_chembl.csv`), uses RDKit to calculate descriptors (like molecular weight, LogP) and Morgan fingerprints for each molecule, and then stores this processed, structured data into a DuckDB database (`chembl_data.duckdb`).

1.  **Set up the Environment**
    ```bash
    # Create and activate a Python virtual environment
    python -m venv venv
    source venv/bin/activate  # on Windows: venv\Scripts\activate

    # Install all dependencies
    pip install -r requirements.txt
    ```

2.  **Run the Data Import Script**
    Execute the script located in the `scripts/` directory to build the database.
    ```bash
    python scripts/import_chembl.py
    ```
    After the script runs successfully, you will see a `chembl_data.duckdb` file in the project's root directory. This step **only needs to be performed once**.

### Phase 2: Launch the API Service

Once the database is built, you can start the FastAPI application at any time to provide services. The API will directly read from the existing `chembl_data.duckdb` file to perform queries.

1.  **Start the Server**
    Run the following command from the project's root directory:
    ```bash
    # uvicorn app.main:app --reload
    # app.main: The `main.py` file inside the `app` directory
    # app: The FastAPI instance `app = FastAPI()` defined in `main.py`
    # --reload: Automatically restart the server on code changes, useful for development
    uvicorn app.main:app --reload
    ```

2.  **Access the API**
    Once the service is running, you can access the following URLs:
    * **Interactive API Docs (Swagger UI)**: [http://1227.0.0.1:8000/docs](http://127.0.0.1:8000/docs)
    * **Alternative API Docs (ReDoc)**: [http://127.0.0.1:8000/redoc](http://127.0.0.1:8000/redoc)

    You can use the interactive documentation to test all API endpoints, such as `/search` and `/resolve_name`.
