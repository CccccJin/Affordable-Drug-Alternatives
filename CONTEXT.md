...
>
> 禁令约束**代码标识符与用户可见文本**。它不约束导出载荷的压缩键（`p`、`u`、`sv`）：
> 那些键脱离 schema 本就没有含义，不会在截图或图例里冒充成一个金额。

## 分组

以下三者形状相同（一组彼此相关的产品）但成员资格规则完全不同，
**其计数不可相加**：把三者之和当作总覆盖是错的。

**Equivalence Group**（等效组）:
Orange Book 中被判定治疗等效的一组产品。这是 FDA 的等效判定。
_Avoid_: group（裸用）

**Biologic Family**（生物药家族）:
Purple Book 中同一参照产品及其 351(k) 后续产品。每个后续产品只对参照产品有评级，
彼此之间没有。
_Avoid_: family（裸用）、group

**ATC Class**（ATC 类）:
共享 WHO ATC 四级化学亚组的一组物质。这是**分类，不是等效判定**——
成员在效价与给药上各不相同，彼此不可替代。
_Avoid_: class（裸用）、group
